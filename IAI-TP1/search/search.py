...
def uniformCostSearch(problem):
    ...
    "getCostOfActions dada una lista de acciones me devuelve su costo"    
    initial_state = problem.getStartState()
    initial_actions = []
    initial_candidate = (initial_state, initial_actions)
    prioridad_primero = problem.getCostOfActions(initial_actions)    
    fringe.push(initial_candidate, prioridad_primero)
    closed_set = set()
    while not fringe.isEmpty():
        candidate = fringe.pop()
        state, actions = candidate
        ...
        if state not in closed_set:
            ...
            for candidate in candidate_successors:
                prioridad = problem.getCostOfActions(actions)
                fringe.push(candidate, prioridad)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    initial_actions = []
    initial_candidate = (initial_state, initial_actions)
    prioridad_primero = problem.getCostOfActions(initial_actions) + heuristic(initial_state, problem)    
    fringe.push(initial_candidate, prioridad_primero)
    closed_set = set()
    while not fringe.isEmpty():
        candidate = fringe.pop()
        ...
        if state not in closed_set:
            ...
            for candidate in candidate_successors:
                prioridad = problem.getCostOfActions(actions) + heuristic(state, problem) 
                fringe.push(candidate, prioridad)
